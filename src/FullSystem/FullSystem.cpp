...
// #include "pclProcessor/pclProcessor.h"
...
namespace dso
{
	...
	FullSystem::~FullSystem()
	{
		...
		// 删除掩模数据，释放内存
		for (FrameShell *ms : allMasksHistory)
			delete ms;
		for (FrameHessian *fh : segmentMasks)
			delete fh;
	...
	void FullSystem::activatePointsMT()
	{
		...
		if (multiThreading)
			// boost::bind返回的是一个函数对象，假如被绑定的函数有n个参数，bind中只绑定了k个，其余n-k个用占位符，那返回的是一个仅有n-k个参数的函数对象
			// 例如f（a,b,c）,G=boost::bind(&f,a,b,_1),G等价于函数G（x）,其中x就是占位符_1表示的参数
	...
	void FullSystem::addActiveFrame(ImageAndExposure *image, int id, ImageAndExposure *mask)
	{
		...
		// addSegmentationMasks(mask, id);

		// debug输出，观察shell-> and shell->incoming_id的区别
		// std::cout << "frame id=" << shell->id << " frame_incoming_id=" << shell->incoming_id << endl;
		...
		else // do front-end operation.
		{
			...
			if (needToMakeKF)
			{
				// 导入分割掩模
				currentMask = mask;
				pointProjectOntoCurrentFH(fh); // 把关键帧上的点全部投影至当前帧
			}
	...
	void FullSystem::makeKeyFrame(FrameHessian *fh)
	{
		...
		allKeyFramesHistory.push_back(fh->shell); // 在addactivefame不是已经把该帧加入allkeyframeHistory了吗——这是all Key FramesHistory
		...
		// 这个大括号是给互斥锁coarseTrackerSwapMutex提供作用域的，超过作用域就自动解锁
		{
			boost::unique_lock<boost::mutex> crlock(coarseTrackerSwapMutex);
	...
	void FullSystem::CurrentViewPoints(FrameHessian *fh)
	{
		if (!initialized)
			return;
		// 创建空白帧
		cv::Mat CurrentFrame(cv::Size(wG[0], hG[0]), CV_8UC3, cv::Scalar(255, 255, 255));
		// 把keyframe的pointHessian全部投影到上述空白帧
		SE3 cam2world_Cur = fh->shell->camToWorld;
		std::vector<cv::Vec6f> pointXYZRGB;

		std::ofstream file("../pointcoordinates.txt", std::ios::app);
		if (!file.is_open())
		{
			std::cout << "failed to open PointsCoordinates.txt" << std::endl;
		}
		for (FrameHessian *f : frameHessians)
		{
			SE3 cam2world_Ref = f->shell->camToWorld;
			...
			SE3 Ref_2_Cur = cam2world_Cur.inverse() * cam2world_Ref;
			for (PointHessian *p : f->pointHessians)
			{
				// 仅仅投影到世界坐标系
				// float depth = 1 / p->idepth_scaled;
				// Eigen::Vector3f Point_Ref(p->u, p->v, 1);
				// Eigen::Vector3f Point_world_ = (cam2world_Ref * (Ki * Point_Ref * depth).cast<double>()).cast<float>();
				// float *depth_Cur = &Point_Cur_[2];
				// Eigen::Vector3f Point_Cur = K * (Point_Cur_ / *depth_Cur);
				// pointXYZRGB.push_back(cv::Vec6f(Point_world_[0], Point_world_[1], Point_world_[2], 50, 200, 10));

				float depth = 1 / p->idepth_scaled;
				Eigen::Vector3f Point_Ref(p->u, p->v, 1);
				Eigen::Vector3f Point_Cur_ = (Ref_2_Cur * (Ki * Point_Ref * depth).cast<double>()).cast<float>();
				float *depth_Cur = &Point_Cur_[2];
				Eigen::Vector3f Point_Cur = K * (Point_Cur_ / *depth_Cur);
				if (Point_Cur[0] < wG[0] && Point_Cur[0] >= 0 && Point_Cur[1] < hG[0] && Point_Cur[1] > 0)
				{

					// use deferernt color to denote deferent depth
					float ColorFactor = *depth_Cur / 4.0;
					if (ColorFactor > 1)
						continue;
					float chanB, chanR, chanG;
					chanB = 255 * (Point_Cur[0] / 400.);
					chanR = 255 * (1 - ColorFactor);
					chanG = 255 * ColorFactor;
					CurrentFrame.at<cv::Vec3b>(Point_Cur[1], Point_Cur[0]) = cv::Vec3b(chanB, chanG, chanR);
					CurrentFrame.at<cv::Vec3b>(Point_Cur[1], Point_Cur[0] + 1) = cv::Vec3b(chanB, chanG, chanR);
					CurrentFrame.at<cv::Vec3b>(Point_Cur[1] + 1, Point_Cur[0] + 1) = cv::Vec3b(chanB, chanG, chanR);
					CurrentFrame.at<cv::Vec3b>(Point_Cur[1] + 1, Point_Cur[0]) = cv::Vec3b(chanB, chanG, chanR);
					pointXYZRGB.push_back(cv::Vec6f(Point_Cur_[0], Point_Cur_[1], *depth_Cur, chanR, 0, chanB));
				}
				// }
			}
			// file.close();
			// color_based_RegionGrowing(pointXYZRGB);

			cv::imshow("current frame with all point", CurrentFrame);
			// 	// if (lines.size() > 0)
			cv::waitKey(20);
		}
	}

	void FullSystem::pointProjectOntoCurrentFH(FrameHessian *fh)
	{
		if (!initialized)
			return;
		// 创建空白帧
		...
		// 检查 fh 和 shell 指针的有效性
		if (fh == nullptr || fh->shell == nullptr)
		{
			std::cerr << "Frame handler or shell is null" << std::endl;
			return;
		}

		if (!isSE3Valid(fh->shell->camToWorld))
		{
			std::cerr << "camToWorld SE3 is not properly initialized" << std::endl;
			return;
		}

		// 检查 cam2world_Cur 的有效性
		if (fh->shell->camToWorld.unit_quaternion().norm() < 1e-6)
		{
			std::cerr << "Invalid cam2world_Cur SE3 after assignment" << std::endl;
			return;
		}
		// 把keyframe的pointHessian全部投影到上述空白帧
		SE3 cam2world_Cur = fh->shell->camToWorld;
		...
		// 点云
		std::vector<Eigen::Vector4f> points; // 获得分割属性的点，第4维是分类属性

		std::map<float, std::vector<Eigen::Vector4f>> classifiedPoints;
		...
		for (FrameHessian *f : frameHessians)
		{
			...
			for (PointHessian *p : f->pointHessians)
			{
				// project all points onto current keyframe
				float depth = 1 / p->idepth_scaled;																  // get depth of point
				Eigen::Vector3f Point_Ref(p->u, p->v, 1);														  // get regular coordinate
				Eigen::Vector3f Point_Cur_ = (Ref_2_Cur * (Ki * Point_Ref * depth).cast<double>()).cast<float>(); // projection
				float *depth_Cur = &Point_Cur_[2];
				Eigen::Vector3f Point_Cur = K * (Point_Cur_ / *depth_Cur);

				if (Point_Cur[0] < wG[0] && Point_Cur[0] >= 0 && Point_Cur[1] < hG[0] && Point_Cur[1] > 0)
				{
					Eigen::Vector4f point_tmp;
					int wP, hP;
					wP = static_cast<int>(Point_Cur[0]);
					hP = static_cast<int>(Point_Cur[1]);
					float cluster_id = static_cast<float>(currentMask->image[wP + hP * wP]); // 检索该点在分割掩模中的值，即该点所属的分割对象
					point_tmp = Eigen::Vector4f(Point_Cur[0], Point_Cur[1], Point_Cur[2], cluster_id);
	...
	void FullSystem::addSegmentationMasks(ImageAndExposure *image, int id)
	{
		if (isLost)
			return;
		boost::unique_lock<boost::mutex> lock(trackMutex);
		...
		shell->incoming_id = id;
		fh->shell = shell;
		allMasksHistory.push_back(shell);

		// =========================== make Images / derivatives etc. =========================
		fh->ab_exposure = image->exposure_time;
		fh->makeImages(image->image, &Hcalib);
		segmentMasks.push_back(fh);
	}

	bool FullSystem::isSE3Valid(const Sophus::SE3d &se3)
	{
		// 检查平移部分是否有
		if (se3.translation().array().isNaN().any())
		{
			return false;
		}

		// 检查旋转部分是否有效
		Eigen::Quaterniond quat = se3.unit_quaternion();
		if (std::isnan(quat.norm()) || quat.norm() < 1e-6)
		{
			return false;
		}
		return true;
