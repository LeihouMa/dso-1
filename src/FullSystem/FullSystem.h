...
namespace dso
{
	...
	public:
		...
		void addActiveFrame(ImageAndExposure *image, int id, ImageAndExposure *mask);
		...
		void CurrentViewPoints(FrameHessian *fh);

		void pointProjectOntoCurrentFH(FrameHessian *fh);

		// SE3正确初始化判断条件
		bool isSE3Valid(const Sophus::SE3d &se3);
	...
	private:
		...
		std::vector<FrameHessian *> frameHessians; // ONLY changed in marginalizeFrame and addFrame. //滑动窗口内的关键帧
		std::vector<PointFrameResidual *> activeResiduals;
		float currentMinActDist;

		// 实例分割掩模
		std::vector<FrameHessian *> segmentMasks;
		// 实例分割掩模历史数据
		std::vector<FrameShell *> allMasksHistory;
		// 掩模传递接口函数
		void addSegmentationMasks(ImageAndExposure *image, int id);
		// 当前关键帧对应的mask
